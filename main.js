...
function start_process() {
    ...
    rl.on('line', (data) => {
        ...
        if (data.charAt(0) == "$") {
            ...
            console.log(data.replace("@debug:", "DEBUG:"))
        } else if (data.startsWith("@warning")) {
            console.warn(data.replace("@warning:", "WARNING"))
        } else if (data.startsWith("@error")) {
            console.error(data.replace("@error:", "ERROR:"))
        } else if (data.startsWith("@run")) {
            console.error(data.replace("@run:", "RUN SCRIPT:"));
            eval(data.replace("@run:", ""));
        } else {
